CLASS zrk_cl_ce_appr_docs DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PROTECTED SECTION.
  PRIVATE SECTION.
    TYPES t_business_data TYPE TABLE OF zrk_ce_appr_docs.
ENDCLASS.



CLASS ZRK_CL_CE_APPR_DOCS IMPLEMENTATION.
...
***filter
    ...
    SELECT FROM zrk_t_pur_con
    FIELDS 'Puchase Contract' AS DocType ,
    object_id AS ObjectId,
    description,
    buyer,
    target_value AS TargetValue,
    currency
    WHERE stat_code = 'AWAP'
    INTO TABLE @DATA(lt_purcon).
    IF sy-subrc EQ 0.
      business_data = CORRESPONDING #( APPENDING ( business_data ) lt_purcon ).
    ENDIF.
